...
sub run_exe_with_pin($$$$) {
  my ($exe, $compiler, $raw_out, $pin_all_addrs_file) = @_;
 
  my $res = runit("$RunSafely $PROG_TIMEOUT 1 /dev/null $raw_out ./$exe");
  if ($res == 0) {
    my $pin_cmd = "$RunSafely $PIN_PROG_TIMEOUT 1 /dev/null $raw_out $PIN_BIN  -all_vars_input $pin_all_addrs_file -- ./$exe";
    print_msg("$pin_cmd\n");
    $res = runit($pin_cmd);
  }
...
sub get_addrs_file($$$$$$) {
  ...
  die "gen_volatile_addr.pl failed!" if runit("$GEN_VOLATILE_ADDR --vars-file=$checker_out --all-vars-file=$all_vars_out --all-var-addrs-output=$all_addrs_file $exe > $PIN_ADDR_FILE 2>&1");
  die "no all_addrs_file!" unless (-f $all_addrs_file);
  filter_globals($all_addrs_file);
  return ($exe, $all_addrs_file);
}

sub get_checksum($$$) {
  my ($compiler, $exe, $pin_all_addrs_file) = @_;

  my $raw_out = "$exe.raw-out";
  my $res;
  die "run exe failed!" if (($res = run_exe_with_pin($exe, $compiler, $raw_out, $pin_all_addrs_file)) != 0);
  if (-f "$PIN_ADDR_FILE") {
...
sub do_one_test($$$) {
  ...
  my ($gcc_exe, $gcc_all_addrs_file) = get_addrs_file("gcc", $root, $cfile, $checker_out, $all_vars_out, $for_unittest);
  if ($for_unittest) {
    my $gcc_checksum = get_checksum("gcc", $gcc_exe, $gcc_all_addrs_file);
    ...
    my ($clang_exe, $clang_all_addrs_file) = get_addrs_file("clang", $root, $cfile, $checker_out, $all_vars_out, $for_unittest);
    my @all_addrs_files = ();
    push @all_addrs_files, $gcc_all_addrs_file;
    push @all_addrs_files, $clang_all_addrs_file;
    uniq_all_addrs_files(\@all_addrs_files);

    my $gcc_checksum = get_checksum("gcc", $gcc_exe, $gcc_all_addrs_file);
    my $clang_checksum = get_checksum("clang", $clang_exe, $clang_all_addrs_file);
...
sub check_prereqs($) {
  ...
  while ($tries > 0) {
    ...
    $test_case_ok = 1;
    my $raw_out = "$exe.raw-out";
    if (run_exe_with_pin($exe, "gcc", $raw_out, $pin_all_addrs_file) == 0) {
      $run_exe_ok = 1;
      last;
    }
  }
