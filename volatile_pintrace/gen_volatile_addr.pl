...
sub process_nm_output($) {
  ...
  while (my $line = <INF>) {
    chomp $line;
    if ($line =~ m/0*([0-9a-fA-F]+)[\s\t]+[bBdD][\s\t](.+)$/) {
      my $addr = hex($1);
      my $name = $2;
...
my $help_msg = '
gen_volatile_addr.pl --address-file=<file> exec
  where:
  exec: executable
  --address-file=<file>: addr file generated by volatile_checker
  --keep-temps: keep the temp files
